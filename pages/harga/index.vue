<template>
    <head>
        <title>Berdasarkan Harga | Pizza Sales Report</title> 
    </head>
...
<script setup>
    ...
    async function fetchDataAndCreateChart() {
        ...
        complete: function (results) {
            ...
            const unitPrices = [...new Set(data.map(item => item.unit_price))];
            ...
            data.forEach(item => {
            dataValues[item.pizza_name][item.unit_price] = (dataValues[item.pizza_name][item.unit_price] || 0) + item.quantity;
            });
    
            createChart(pizzaNames, unitPrices, dataValues);
        }
        });
    }
