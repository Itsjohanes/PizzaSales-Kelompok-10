<template>
    <head>
        <title>Vegetarian dan Non-Vegetarian | Pizza Sales Report</title> 
    </head>
    <div class="my-5 bg-white rounded-2xl font-semibold text-center text-sm text-gray-600 p-5 shadow-md w-full">
        Penjualan Berdasarkan Kategori Vegetarian dan Non-Vegetarian
...
<script setup>
    ...
    async function fetchDataAndCreateChart() {
        ...
        complete: function (results) {
            ...
            const pizzaCategories = [...new Set(data.map(item => item.pizza_category))];
            ...
            data.forEach(item => {
            dataValues[item.pizza_name][item.pizza_category] = (dataValues[item.pizza_name][item.pizza_category] || 0) + item.quantity;
            });
    
            createChart(pizzaNames, pizzaCategories, dataValues);
        }
        });
    }
